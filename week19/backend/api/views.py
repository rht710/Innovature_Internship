...
class ProductImageViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not images:
            return Response({"image": ["No files were uploaded."]}, status=400)
            
        # Always explicitly create with product_id (single or multiple)
        created_objects = []
        ...
        if len(created_objects) == 1:
            return Response(created_objects[0], status=201)
...
class RegisterView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            # Create standard token for compatibility
